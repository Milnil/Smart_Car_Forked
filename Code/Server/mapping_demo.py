...
class Mapping:
    ...
    def move_car(self, new_position):
        ...
        motor = self.mock_motor # Change to self.motor when running actual test
        ...
        if abs(angle_diff) > 10:  # Threshold for rotation
            if angle_diff > 0:
                ...
                motor.setMotorModel(2000, 2000, -2000, -2000)  # Rotate right
                time.sleep(
                1)
            else:
                print("rotating left")
                motor.setMotorModel(-2000, -2000, 2000, 2000)  # Rotate left
                time.sleep(
                        1.5)  # Adjust sleep time based on angle difference
            print(f"Angle diff: {angle_diff}")
            motor.setMotorModel(0, 0, 0, 0)  # Stop rotation
            ...
            motor_speed = 1000
            motor.setMotorModel(motor_speed, motor_speed, motor_speed, motor_speed)
            time.sleep(distance * 0.1)  # Adjust this factor based on your car's speed
            motor.setMotorModel(0, 0, 0, 0)  # Stop movement
